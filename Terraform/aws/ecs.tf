...
resource "aws_ecs_task_definition" "backend" {
  ...
  container_definitions = jsonencode([
    {
      name      = "backend"
      image     = var.backend_docker_image != "" ? var.backend_docker_image : "${aws_ecr_repository.backend.repository_url}:latest"
      essential = true
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "wget --no-verbose --tries=1 --spider http://localhost:${var.backend_container_port}/docs || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 60
...
resource "aws_ecs_task_definition" "frontend" {
  ...
  container_definitions = jsonencode([
    {
      name      = "frontend"
      image     = var.frontend_docker_image != "" ? var.frontend_docker_image : "${aws_ecr_repository.frontend.repository_url}:latest"
      essential = true
...
resource "aws_ecs_service" "frontend" {
  ...
  depends_on = [
    aws_lb_listener.http,
    aws_lb_target_group.frontend_v2
  ]
