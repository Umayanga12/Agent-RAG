...
from .services.streaming_service import stream_qa_response
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        "*",  # Allow all origins for local development access
    ],
...
async def qa_stream_endpoint(payload: QuestionRequest):
    """Stream QA response with real-time agent progress via SSE.
    
    This endpoint:
    - Streams agent execution stages (planning, retrieval, summarization, verification)
    - Emits events following Vercel AI SDK Data Stream Protocol
    - Provides real-time progress updates to the frontend
    - Returns final answer token-by-token
    """
    question = payload.question.strip()
    if not question:
        raise HTTPException(
